...
use volatile_register::{RW, WO, RO};


register!(
    /// `master_enable` is the master interrupt enable/disable flag.
    pub master_enable: RW<MasterEnable> => 0x4000208);
...
register!(
    ...
    pub enable: RW<SourceSet> => 0x4000200);
...
register!(
    ...
    pub sources: RO<SourceSet> => 0x4000202);
...
register!(
    ...
    /// }
    /// ```
    pub irq_acknowledge: WO<SourceSet> => 0x4000202);
